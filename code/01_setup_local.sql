...
select 'spin.tsv'      file_name, 'SPIN_STAGING'      staging_table_name,    70 min_expected_rows, 'https://planet4589.org/space/gcat/tsv/worlds/spin.tsv'      url, '#IDT	IDName	Name	Rho	IFac	PoleRA	PoleDec	Meridian	SpinRate	J2	J4	J6	PoleRARate	PoleDecRate	PoleFunc	SpinFunc	InitFunc	JFile' from dual union all
...
select 'ptmpcat.tsv'   file_name, 'PTMPCAT_STAGING'   staging_table_name,     1 min_expected_rows, 'https://planet4589.org/space/gcat/tsv/cat/ptmpcat.tsv'      url, '#JCAT	Piece	Name	LDate	TLast	TOp	TDate	TF	Program	Plane	Att	Mvr	Class	Category	Result	Control	Discipline	UNState	UNReg	UNPeriod	UNPerigee	UNApogee	UNInc	DispEpoch	DispPeri	DispApo	DispInc	Comment' from dual union all
