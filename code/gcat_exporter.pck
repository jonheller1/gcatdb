...
procedure generate_oracle_file is
	...
	begin
		...
		q'[
				...
				exec dbms_output.put_line('-- The database installs 20 tables and uses about ??(TODO)MB of space.');
				exec dbms_output.put_line('--');
...
procedure generate_postgres_file is
	...
	function get_handle(p_open_mode in varchar2) return utl_file.file_type is
		v_handle utl_file.file_type;
	begin
		v_handle := utl_file.fopen(c_export_directory, 'postgres_create_space.sql', open_mode => p_open_mode);
		return v_handle;
	end get_handle;
	...
	begin
		...
		for i in 1 .. gcat_helper.c_ordered_objects.count loop
			...
			--Metadata for tables:
			v_metadata := get_metadata('TABLE', 'SPACE'/*temp*/, gcat_helper.c_ordered_objects(i)) || ';';
